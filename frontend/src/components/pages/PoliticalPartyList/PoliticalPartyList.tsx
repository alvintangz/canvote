...
export default function PoliticalPartyCreate(props) {
  ...
  const { loading, error, data } = useQuery(GET_POLITICAL_PARTY, {
    errorPolicy: 'all',
  });
  const [deletePoliticalParty] = useMutation(DELETE_POLITICAL_PARTY, {
    errorPolicy: 'all',
  });
    ...
    <div>
      {data ? (
        <div>
          ...
          <Link to="/manage/parties/new" className="btn-success btn">
            New Party
          </Link>
