...
def create_jwt_for_user_session(user: UserInDB) -> str:
    ...
    :return: JWT token
    """
    logging.debug('Creating a JWT for user session - ', user.email)
    return jwt.encode(
...
def retrieve_current_user_from_jwt_for_user_session(token: str) -> CurrentUser:
    ...
    payload = jwt.decode(token, key=config.JWT_SESSION_SECRET_KEY, algorithm='HS256')
    logging.debug('Retrieved current user - ', payload['email'])
    return CurrentUser(
        email=payload['email'],
...
def create_jwt_for_account_activation(email: str) -> str:
    ...
    :return: JWT token
    """
    logging.debug('Creating a JWT for account activation - ', email)
    return jwt.encode(
...
def retrieve_email_from_jwt_for_account_activation(token: str) -> str:
    ...
    try:
        payload = jwt.decode(token, key=config.JWT_ACTIVATION_SECRET_KEY, algorithm='HS256')
    except Exception as e:
        # TODO
        logging.debug('Swag')
        raise e
    logging.debug('Retrieved user - ', payload['email'])
